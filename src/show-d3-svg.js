...
class ShowD3SVG extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (this.props.routers !== prevProps.routers) {
      ...
      // check for any new clients or edges
      // see if every router is present in the nodes
      const prev = JSON.stringify(prevProps);
      const curr = JSON.stringify(this.props);
      console.log(prevProps);
      console.log(this.props);
      console.log(`componentDidUpdate: test is ${prev !== curr}`);
    }
  }

  addClients = state => {
    console.log("addClients called");
